...
class QuizUI:
    ...
    def give_feedback(self, is_right):
        if is_right:
            self.canvas.config(bg="green")  # Use config for canvas background
        else:
            self.canvas.config(bg="red")  # Use config for canvas background
        self.window.after(1000, self.get_next_question)  # Proceed after feedback